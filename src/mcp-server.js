...
const SERVER_INSTRUCTIONS = [
  ...
  "Use forge_scan, forge_understand, or forge_walk first for broad prompts like understanding the whole repo or monorepo, going through every file or folder, mapping packages, or finding important files. forge_scan is the fastest first-pass repo map. forge_understand auto-escalates for exhaustive prompts, and forge_walk now performs a local file-body audit for explicit every-file requests before returning a compact package-by-package and folder-by-folder digest. After using forge_scan, forge_understand, or forge_walk, answer the initial repo-overview question from those results instead of manually reading lots of files or spawning Explore agents unless the user explicitly asks for a deeper drilldown. For compact file and shell operations inside the current repository, prefer forge_read, forge_write, forge_edit, and forge_bash over heavier built-in tool paths when they are sufficient. Use forge_search for behavior or file lookup, forge_symbol for exact symbol names, forge_scope for architecture questions, forge_impact for blast radius, forge_why for repo-plus-session causality, and forge_resume or forge_session for continuity."
].join(" ");

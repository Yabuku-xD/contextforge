...
[![Release](https://img.shields.io/badge/release-v0.1.33-C2410C?style=for-the-badge)](https://github.com/Yabuku-xD/contextforge/releases)
...
- Stores full text-file bodies with line coverage in the local index, so whole-repo answers can distinguish between "read locally" and "fully remembered in indexed memory"
- Improves retrieval for architecture, search, scope, and blast-radius questions
- Preserves continuity across longer sessions so work does not keep resetting, with indexed session recall that now matches the built-in `context_mode` benchmark suite on this repo's open track
- Steers Claude away from noisy broad repo crawling and back toward compact `forge_*` flows through ContextForge-branded routing hooks
- Adds a ContextForge-native low-context research lane through `forge_batch` and `forge_lookup`, so large command output can stay in local indexed storage instead of bloating chat

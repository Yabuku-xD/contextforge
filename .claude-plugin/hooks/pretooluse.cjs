#!/usr/bin/env node
"use strict";
// Hook launcher stub. Committed alongside the plugin so the hook works on a
// fresh GitHub install where dist/ has not been built yet. Resolves the real
// hook script from either the local checkout dist/ or the cached runtime
// install used by the inline MCP launcher, then spawns it with passthrough
// stdio so the hook contract is preserved.
const fs = require("node:fs");
const os = require("node:os");
const path = require("node:path");
const cp = require("node:child_process");

const HOOK_RELATIVE = path.join("dist", "hooks", "pretooluse.js");
